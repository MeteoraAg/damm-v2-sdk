...
describe("Remove liquidity & Close position", () => {
  describe.each(poolModes)(
    "Remove all liquidity and close position with SPL-Token ($label)",
    ({ collectFeeMode, compoundingFeeBps }) => {
      ...
      it("Success case", async () => {
        ...
        await executeTransaction(context.banksClient, addLiquidityTx, [
          creator,
        ]);
        ...
        await executeTransaction(context.banksClient, closePositionTx, [
          creator,
        ]);
      });
    },
  ...
  describe.each(poolModes)(
    "Remove all liquidity and close position with Token 2022 ($label)",
    ({ collectFeeMode, compoundingFeeBps }) => {
      let context: ProgramTestContext;
      let payer: Keypair;
      let creator: Keypair;
