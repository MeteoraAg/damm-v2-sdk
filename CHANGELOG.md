...
- Fixed `mergePosition` failing on compounding pools with an "insufficient funds" token transfer error in the final `addLiquidity` instruction. The re-deposit liquidity delta was computed against the pre-removal pool state, but on-chain the add executes after the removal and prices the delta (rounded up) against the post-removal reserves and liquidity, which could require more tokens than the removal returned (rounded down). The delta is now computed against the post-removal pool state.

### Added

- Added optional `isSkipReward` to `MergePositionParams` and `RemoveAllLiquidityAndClosePositionParams`, forwarded to the embedded `claimReward` instructions for pools with frozen reward vaults (skipping forfeits those rewards).

### Changed

- `mergePosition` and `removeAllLiquidityAndClosePosition` omit the reward claim (and reward token account creation) for a reward slot when the campaign is fully settled through `rewardDurationEnd` and the position has no pending rewards with an up-to-date reward checkpoint, since such a slot can never accrue again inside the transaction.
...
- Fixed `validateFeeRateLimiter` function to order.
